...
public class WhdslGenerator extends AbstractGenerator {
  ...
  public void doGenerate(final Resource resource, final IFileSystemAccess2 fsa, final IGeneratorContext context, final String entree, final String sortie, final String indent_value, final String indent_if, final String indent_for, final String indent_while) {
    WhdslStandaloneSetup _whdslStandaloneSetup = new WhdslStandaloneSetup();
    final Injector injector = _whdslStandaloneSetup.createInjectorAndDoEMFRegistration();
    final XtextResourceSet resourceSet = injector.<XtextResourceSet>getInstance(XtextResourceSet.class);
    final URI uri = URI.createURI(entree);
  ...
  public CharSequence compile(final Command c) {
    ...
    if (!_matched) {
      if (((!Objects.equal(c.getVars(), null)) && (!Objects.equal(c.getExprs(), null)))) {
        _matched=true;
    ...
    if (!_matched) {
      ...
      boolean _notEquals_3 = (!Objects.equal(_cmdWhile, null));
      if (_notEquals_3) {
        _matched=true;
